...
fn cmix(coords: &Vec<(usize, i64)>, n: usize) -> i64 {
    ...
    for _ in 0..n {
        for (orig_pos, number) in coords.iter() {
            let from_idx = mix_buffer.iter().position(|x| *x == (*orig_pos, *number)).unwrap();
    ...
    return [1000,2000,3000].iter().map(|o| mix_buffer[(zero_idx + o) % mix_buffer.len()].1).sum();
}
...
fn solve(numbers: &[i64], n: usize) -> i64 {
    let mut coords = numbers.iter().enumerate().map(|(i, n)| Coord{ value: *n, prev: if i == 0 {0} else {(i - 1) as usize}, next: i +1 }).collect::<Vec<_>>();
...
fn main() -> io::Result<()> {
    let stdin = io::stdin();
    let numbers = stdin.lines().map(|l| l.unwrap().parse::<i64>().unwrap()).collect::<Vec<_>>();
