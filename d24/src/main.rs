    ...
    while let Some(current) = heap.pop() {
        ...
        for yd in -1..=1_i32 {
            for xd in -1..=1_i32 {
                ...
                let next = State { time: current.time + 1, p: next_pos};
...
fn print_map(size: Vec2, e: Vec2, t: i32, blizzards: &HashSet<(char, i32, i32)>) {
    ...
    for y in 0..size.y {
        print!("#");
        for x in 0..size.x {
            let b = get_blizzard(&State{time: t, p: Vec2 { x, y }}, size, blizzards);
            if e == (Vec2 { x, y }) {
                if b != '.' {
...
fn main() -> io::Result<()> {
    ...
    let p1 = dijkstra(State{time: 0, p:start}, goal, size, &blizzards);
    ...
    let p2 = dijkstra(State{time: p1.len() as i32, p:start}, goal, size, &blizzards);
    println!("start<-goal takes {} Minutes", p2.len());

    let start = Vec2 { x: 0, y: -1};
    let goal = Vec2 { x: size.x - 1, y: size.y - 1};
    let p3 = dijkstra(State{time: (p1.len() + p2.len()) as i32, p:start}, goal, size, &blizzards);
